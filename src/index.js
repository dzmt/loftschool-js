...
function findAllPSiblings(where) {
    ...
    for (let element of where.children) {
        let hasSiblingElement = element.nextElementSibling ? true : false;

        if (hasSiblingElement && element.nextElementSibling.tagName === 'P') {
...
function deleteTextNodes(where) {
    let length = where.childNodes.length;
...
function collectDOMStat(root) {
    ...
    if (tree.nodeType === 1) {
        let count = stat.tags[tree.tagName];

        stat.tags[tree.tagName] = count ? count + 1 : 1;

        tree.classList.forEach(c => {
            stat.classes[c] = stat.classes[c] ? stat.classes[c] + 1 : 1;
        });
        
        tree.childNodes.forEach(e => {
            collectDOMStat(e, stat, tree, true);
        });
    }
